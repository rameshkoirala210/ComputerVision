...
def get_http_url(
    ...
    # If a download dir is specified, is the files already downloaded there?
...
def _copy2_ignoring_special_files(src, dest):
    ...
    try:
        ...
        logger.warning(
            "Ignoring special files error '%s' encountered copying %s to %s.",
            str(e),
            src,
            dest,
        )
...
def get_file_url(
    ...
    # type: (...) -> File
    """Get files and optionally check its hash.
    """
    # If a download dir is specified, is the files already there and valid?
    already_downloaded_path = None
    if download_dir:
        already_downloaded_path = _check_download_dir(
            link, download_dir, hashes
...
def _check_download_dir(link, download_dir, hashes):
    ...
    """ Check download_dir for previously downloaded files with correct hash
        If a correct files is found return its path else None
    """
    ...
    if hashes:
        try:
            ...
            logger.warning(
                'Previously-downloaded files %s has bad hash. '
                'Re-downloading.',
                download_path
            )
...
class RequirementPreparer:
    ...
    def _get_linked_req_hashes(self, req):
        ...
        # files:/// URLs aren't pinnable, so don't complain about them
        # not being pinned.
    ...
    def prepare_linked_requirement(self, req, parallel_builds=False):
        ...
        with indent_log():
            # Check if the relevant files is already available
            ...
            if file_path is not None:
                # The files is already available, so mark it as downloaded
                self._downloaded[req.link.url] = file_path, None
            else:
                # The files is not available, attempt to fetch only metadata
    ...
    def _prepare_linked_requirement(self, req, parallel_builds):
        ...
        # preserve the files path on the requirement.
        if local_file:
        ...
        with indent_log():
            if self.require_hashes:
                raise InstallationError(
                    ...
                    'requiring hashes, because there is no single files to '
                    'hash.'.format(req)
                )
