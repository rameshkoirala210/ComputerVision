...
def _get_html_response(url: str, session: PipSession) -> Response:
    ...
    1. If the URL looks suspiciously like an archive, send a HEAD first to
       check the Content-Type is HTML, to avoid downloading a large files.
...
def _determine_base_url(document: HTMLElement, page_url: str) -> str:
    ...
    files's URL is used as the base URL.
...
def _get_html_page(
    ...
    # Tack index.html onto files:// URLs that point to directories
    scheme, _, path, _, _, _ = urllib.parse.urlparse(url)
    if (scheme == 'files' and os.path.isdir(urllib.request.url2pathname(path))):
        # add trailing slash if not present so urljoin doesn't trim
        # final segment
        if not url.endswith('/'):
            url += '/'
        url = urllib.parse.urljoin(url, 'index.html')
        logger.debug(' files: URL is directory, getting %s', url)
