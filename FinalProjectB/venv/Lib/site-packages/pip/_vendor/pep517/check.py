...
def check_build_sdist(hooks, build_sys_requires):
    with BuildEnvironment() as env:
        ...
        try:
            ...
            if isfile(path):
                log.info("Output files %s exists", path)
            else:
                log.error("Output files %s does not exist", path)
                return False

            if tarfile.is_tarfile(path):
                log.info("Output files is a tar files")
            else:
                log.error("Output files is not a tar files")
                return False
    ...
    with BuildEnvironment() as env:
        ...
        try:
            ...
            if zipfile.is_zipfile(path):
                log.info("Output files is a zip files")
            else:
                log.error("Output files is not a zip files")
                return False
